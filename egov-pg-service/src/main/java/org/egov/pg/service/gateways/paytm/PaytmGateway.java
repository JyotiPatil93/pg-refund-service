...
/*
		...
		treeMap.put("orderId", transaction.getTxnId());
		treeMap.put("txnId", transaction.getGatewayTxnId());
		treeMap.put("refId", transaction.getTxnId());
		treeMap.put("refundAmount", transaction.getRefundAmount());*/

		try {
			String checkSum = PaytmChecksum.generateSignature(body.toString(), MERCHANT_KEY);
			JSONObject head = new JSONObject();
			head.put("signature", checkSum);
			//treeMap.put("signature", checkSum);
			...
			PaytmRefundResponse paytmRefundResponse = objectMapper.convertValue(jsonObject.get("body"),
					PaytmRefundResponse.class);
